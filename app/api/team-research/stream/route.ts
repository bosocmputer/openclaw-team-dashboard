        ...
        for (const agent of selectedAgents) {
          ...
          try {
            const result = await callLLM(agent.provider, agent.model, apiKey, agent.baseUrl, [
              ...
              {
                role: "user",
                content: `คำถาม: ${question}\n\nความเห็นของคุณก่อนหน้า:\n${myFinding.content}\n\nความเห็นของ agents อื่น:\n${otherFindings}\n\nกรุณาวิเคราะห์ความเห็นของ agents อื่น แล้วแสดงความคิดเห็นเพิ่มเติม หรือเห็นด้วย/ไม่เห็นด้วยในประเด็นใด พร้อมเหตุผล`,
              },
            ]);
          ...
          const result = await callLLM(synthAgent.provider, synthAgent.model, synthApiKey, synthAgent.baseUrl, [
            {
              role: "system",
              content:
                "คุณคือผู้สรุปผล กรุณาสรุปคำตอบที่ดีที่สุดจากข้อมูลทั้งหมดที่ agents ได้วิเคราะห์มา โดยให้ครบถ้วน ชัดเจน และนำไปใช้งานได้จริง",
            },
            {
              role: "user",
